...
(lp/addclause '((member-and-rest A (cs B L1) (cs B L2)) :- (member-and-rest A L1 L2)))
...
(lp/addclause '((nth-item-and-rest zero A Lst Rest) :- (member-and-rest A Lst Rest)))

(lp/addclause '((nth-item-and-rest (succ zero) A (cs A Rest) Rest)))
(lp/addclause '((nth-item-and-rest (succ N) A (cs B Tail) (cs B Rest)) :- (nth-item-and-rest N A Tail Rest)))

(lp/addclause '((member-move-to-end A (cs A Rest) NewList) :- (add-to-end A Rest NewList)))
(lp/addclause '((member-move-to-end (cs B Tail) (cs B NewList)) :- (add-to-end A Rest NewList)))


(lp/addclause '((add-to-end A ni (cs A ni))))
(lp/addclause '((add-to-end A (cs Head Tail) (cs Head Newtail)) :- (add-to-end A Tail Newtail)))
